...
describe('E2E tests', function () {
  ...
  describe('Messenger Info', () => {
    it('Load Message', async () => {
      ...
      const loadedText = await page.evaluate(() => document.querySelector('textarea').value);
      const data =  mockData.list
        .map(msg => `${msg.author}: ${msg.content}`)
        .join('\n');

      expect(loadedText).to.equal(data);
    });

    it('Send Message API call', async () => {
      const user = {
        author: 'Pesho',
        content: 'Good morning fellas'
      }

      await page.goto(host);
      await page.waitForSelector('input[name="author"]');
      await page.waitForSelector('input[name="content"]');
      await page.waitForSelector('#submit');

      await page.fill('input[name="author"]', user.author);
      await page.fill('input[name="content"]', user.content);
      await page.click('#submit');

      mockData.list.push(user);
